import {LinkMessageEvent} from "./LinkMessageEvent";
...
function isLinkMessageEvent(obj: Event): obj is LinkMessageEvent {
  return obj instanceof LinkMessageEvent
}
...
export class InterConnectService {
    ...
    return this.__net.createServer((socket: any) => {
      ...
      socket.on('data', (data: any) => {
        ...
        try {
          ...
          dataBuffer = '';
          try {
            new LinkMessageEvent(message, socket).dispatch();
          } catch (e: any) {
   ...
   * @param event 事件
   */
  addServiceEventListener(type: string, event: (e: LinkMessageEvent) => void) {
